...
def run_scraper():
    ...
    # Lê as URLs do arquivo filtrando linhas vazias e comentários
    with open(caminho_arquivo_urls, 'r', encoding='utf-8') as f:
        urls_para_analisar = [linha.strip() for java_linha in f if (java_linha := java_linha.strip()) and not java_linha.startswith('#')]

    # Ajuste simples de segurança para a lista de URLs
    urls_para_analisar = [linha.strip() for linha in urls_para_analisar if linha.strip()]
